...
function dashboardPage(users, config) {
  ...
  return wrapHTML(
    ...
    '<div class="card"><div style="color:var(--text2);">👥 کاربران</div><div class="stat-val">' + users.length + '</div><span class="badge badge-ok">' + active + ' فعال</span></div>' +
    ...
    '</div>' +
    '<div class="card fade-in"><h3 class="mb-20">📋 فعالیت اخیر</h3>' + actHTML + '</div>' +
    '<p class="text-center" style="color:var(--text2);margin-top:20px;">🏗️ TAAKAA | @TaaKaaOrg | ⚡ v16 PRO</p>',
    'TAAKAA-XI | داشبورد'
  );
}
...
function usersPage(users) {
  ...
  return wrapHTML(
    ...
    '<div class="card fade-in"><h3 class="mb-20">لیست (' + users.length + ')</h3><div style="overflow-x:auto;"><table><thead><tr><th>نام</th><th>UUID</th><th>IP</th><th>مصرف</th><th>روزانه</th><th>زمان</th><th>وضعیت</th><th>عملیات</th></tr></thead><tbody>' + rows + '</tbody></table></div></div>' +
    '<div id="add-user-modal" class="modal-overlay"><div class="modal fade-in"><h3>➕ ایجاد کاربر</h3><form onsubmit="event.preventDefault();createUser()" id="add-user-form"><input type="text" name="name" placeholder="نام" required><input type="text" name="uuid" placeholder="UUID"><input type="text" name="ip" placeholder="IP"><input type="text" name="quota" placeholder="حجم (5GB)" value="5GB"><input type="text" name="dailyQuota" placeholder="روزانه (1GB)" value="1GB"><input type="text" name="expiry" placeholder="مدت (30d)" value="30d"><select name="operator"><option value="mtn">ایرانسل</option><option value="mci">همراه اول</option><option value="rtl">رایتل</option></select><div class="flex mt-20"><button type="submit" class="btn">✅ ایجاد</button><button type="button" class="btn btn-outline" onclick="closeModal(\'add-user-modal\')">انصراف</button></div></form></div></div>' +
    '<div id="edit-user-modal" class="modal-overlay"><div class="modal fade-in"><h3>✏️ ویرایش</h3><form onsubmit="event.preventDefault();saveEditUser()" id="edit-user-form"><input type="hidden" name="uuid" id="edit-uuid"><input type="text" name="name" id="edit-name" required><input type="text" name="ip" id="edit-ip"><input type="text" name="quota" id="edit-quota"><input type="text" name="dailyQuota" id="edit-dailyQuota"><input type="text" name="expiry" id="edit-expiry"><div class="flex mt-20"><button type="submit" class="btn">💾 ذخیره</button><button type="button" class="btn btn-outline" onclick="closeModal(\'edit-user-modal\')">انصراف</button></div></form></div></div>' +
