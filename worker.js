// ============================================================
// بخش ۱: تنظیمات اصلی - ثابت‌های پروژه
// ============================================================

const PROJECT_NAME = "TaaKaa-Xi";
const DB_NAME = "TAAKAA_DB";
const GITHUB_REPO = "Tentayzm/TaaKaa-Xi";
const TELEGRAM_CHANNEL = "@TaaKaaOrg";
const DEFAULT_PASSWORD = "taakaa";
const CURRENT_VERSION = "3.0.0";
// ============================================================
// بخش ۲: تنظیمات اصلی - مقادیر پیش‌فرض سیستم
// ============================================================

const SYSTEM_DEFAULTS = {
    name: "TaaKaa-Xi",
    apiRoute: "taakaa",
    maintenanceHost: "https://www.nginx.com",
    backupRelay: "",
    customRelay: "",
    masterKey: "taakaa",
    metricNode: "time.is",
...
// ============================================================
// بخش ۴: توابع کمکی - getAlpha, getBeta, getGamma, safeBtoa
// ============================================================
...
// ============================================================
// بخش ۵: دیتابیس - d1Init (مقداردهی اولیه)
// ============================================================

async function d1Init(env) {
    if (env[DB_NAME] && !env[DB_NAME + "_INITIALIZED"]) {
        try {
            await env[DB_NAME].prepare(
                "CREATE TABLE IF NOT EXISTS kv_store (key TEXT PRIMARY KEY, value TEXT)"
            ).run();
            env[DB_NAME + "_INITIALIZED"] = true;
        } catch(e) {
            env[DB_NAME + "_INITIALIZED"] = true;
        }
    }
...
// ============================================================
// بخش ۶: دیتابیس - d1Get (دریافت مقدار)
// ============================================================

async function d1Get(env, key) {
    if (!env[DB_NAME]) return null;
    await d1Init(env);
    try {
        const { results } = await env[DB_NAME]
...
// ============================================================
// بخش ۷: دیتابیس - d1Put و cachedD1Put (ذخیره مقدار)
// ============================================================

async function d1Put(env, key, value) {
    if (!env[DB_NAME]) return;
    await d1Init(env);
    try {
        await env[DB_NAME]
...
// ============================================================
// بخش ۸: دیتابیس - loadSysConfig (بارگذاری تنظیمات)
// ============================================================
...
async function loadSysConfig(env) {
    ...
    if (now - backupIpCacheTime > CACHE_TTL_BACKUP_IP) {
        if (!backupIpLoading) {
            backupIpLoading = (env[DB_NAME] ? d1Get(env, "backup_ip") : Promise.resolve(null)).then(val => {
                backupIpCache = val;
...
// ============================================================
// بخش ۹: رمزنگاری - sha224Hex (تابع هش)
// ============================================================
...
// ============================================================
// بخش ۱۰: رمزنگاری - getTrojanHash و generateHardwareId
// ============================================================
...
// ============================================================
// بخش ۱۲: توابع کمکی پروفایل - getAllProfiles
// ============================================================
...
// ============================================================
// بخش ۱۳: توابع کمکی - getTransportParams و getCleanIps
// ============================================================
...
// ============================================================
// بخش ۱۴: توابع کمکی - getProxyIpsArray و getConfigName
// ============================================================
...
function getConfigName(type, profileName, port, hostName, ip, proxyIp = null) {
    let prefix = sysConfig.namePrefix || "TaaKaa";
...
// ============================================================
// بخش ۱۵: توابع کمکی - getSubscriptionStats و calcEffectiveIps
// ============================================================
...
// ============================================================
// بخش ۱۶: توابع تلگرام - logActivity و fetchCloudflareUsage
// ============================================================

async function logActivity(env, type, detail) {
    if (!env || !env[DB_NAME]) return;
    try {
...
// ============================================================
// بخش ۱۷: توابع تلگرام - sendTelegramMessage
// ============================================================
...
async function sendTelegramMessage(request, type, hostName) {
    ...
    const panelUrl = `https://${h}/${encodeURI(sysConfig.apiRoute)}/taakaa`;
...
// ============================================================
// بخش ۱۸: پیام‌های چندزبانه - botI18n (انگلیسی)
// ============================================================
...
const botI18n = {
    ...
    // ============================================================
// بخش ۱۹: پیام‌های چندزبانه - botI18n (فارسی)
// ============================================================
    ...
    fa: {
        ...
        msg_confirm_panic: "⚠️ آیا از فعال‌سازی وضعیت اضطراری اطمینان دارید؟",
...
// ============================================================
// بخش ۲۰: هندلرهای مسیرها - handleAuth
// ============================================================
...
async function handleAuth(request, hostName, ctx, env) {
    try {
        ...
        if (data.key === sysConfig.masterKey) {
            ...
            if (sysConfig.tgAdminId && env[DB_NAME]) {
                const loginSignal = {
...
// ============================================================
// بخش ۲۲: هندلرهای مسیرها - handleUsersApi و handleStatsApi
// ============================================================
...
// ============================================================
// بخش ۲۳: هندلرهای مسیرها - handleUpdateApi و handleSyncPanel
// ============================================================
...
async function handleUpdateApi(request, env, ctx) {
    try {
        ...
        const repo = (sysConfig.githubRepo || GITHUB_REPO).replace(/https?:\/\/github\.com\//, '').trim();
...
// ============================================================
// بخش ۲۴: هندلرهای مسیرها - handleTelegramWebhook
// ============================================================
...
async function handleTelegramWebhook(request, env, hostName, ctx) {
    try {
        ...
        const getMainMenu = (isAdmin = true) => {
            const isPaused = sysConfig.isPaused || false;
        ...
        if (update.callback_query) {
            ...
            if (chatId) {
                ...
                if (data === "main_menu") {
                    const menu = getMainMenu(isAuthorized);
                    ...
                    if (!u) {
                        await sendOrEdit(chatId, t("lbl_user_not_found"), { inline_keyboard: [[{ text: t("btn_back"), callback_data: "subs_list:0" }]] });
                        return;
                    }
                    ...
                    await fetch(`${tgApi}/answerCallbackQuery`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ callback_query_id: cb.id, text: t("sub_link_sent") })
                    }).catch(()=>{});
            ...
            if (isAuthorized) {
                ...
                if (state) {
                    ...
                    if (state.step === "sub_add_limits") {
                        ...
                        const successMsg = `✅ ${t("msg_added")}`;
                        const kb = { inline_keyboard: [[{ text: t("btn_main_menu"), callback_data: "main_menu" }]] };
                        await sendOrEdit(chatId, successMsg, kb);
                        return new Response("OK", { status: 200 });
                    }
...
// ============================================================
// بخش ۲۵: صفحات HTML، Build Functions، WebSocket و خروجی نهایی
// ============================================================
...
<head>
    ...
    <style>
        body { font-family: 'Vazirmatn', sans-serif; background: #0a0a0f; color: #f1f5f9; }
        .premium-card { background: rgba(26,26,46,0.85) !important; border: 1px solid rgba(255,107,0,0.25) !important; box-shadow: 0 10px 40px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.05) !important; }
        .nav-item.active { background: linear-gradient(90deg, rgba(255,107,0,0.2), transparent) !important; color: #ff8c42 !important; border-inline-start: 4px solid #ff6b00 !important; }
        ::-webkit-scrollbar { width: 6px; }
        ::-webkit-scrollbar-thumb { background: rgba(255,107,0,0.3); border-radius: 10px; }
        .btn-primary { background: linear-gradient(135deg, #ff6b00, #ff8c42); }
        .btn-primary:hover { background: linear-gradient(135deg, #ff8c42, #ff6b00); }
        .text-primary { color: #ff6b00; }
        .border-primary { border-color: #ff6b00; }
        .bg-primary\\/10 { background: rgba(255,107,0,0.1); }
        .bg-primary\\/20 { background: rgba(255,107,0,0.2); }
        .bg-primary\\/5 { background: rgba(255,107,0,0.05); }
    </style>
</head>
<body class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-2xl premium-card rounded-3xl p-8">
        <h1 class="text-3xl font-black text-center text-primary">TaaKaa-Xi Gateway</h1>
        <p class="text-center text-slate-400 mt-2">v${CURRENT_VERSION}</p>
        <div class="mt-8 space-y-4">
            <p class="text-slate-300">✅ پنل با موفقیت راه‌اندازی شد!</p>
            <p class="text-slate-400 text-sm">مسیر پنل: <code class="bg-slate-800 px-2 py-1 rounded text-primary">/${sysConfig.apiRoute}</code></p>
            <p class="text-slate-400 text-sm">رمز پیش‌فرض: <code class="bg-slate-800 px-2 py-1 rounded text-primary">${sysConfig.masterKey}</code></p>
            ${!hasDB ? '<p class="text-amber-400 text-sm">⚠️ دیتابیس متصل نیست - تنظیمات ذخیره نمی‌شوند</p>' : ''}
            <div class="flex flex-wrap gap-3 mt-6">
                <a href="/${sysConfig.apiRoute}/taakaa" class="btn-primary px-6 py-3 rounded-xl font-bold text-white transition-all shadow-lg hover:shadow-xl">📊 ورود به پنل</a>
                <a href="/${sysConfig.apiRoute}/scanner" class="px-6 py-3 rounded-xl font-bold bg-slate-800 hover:bg-slate-700 text-white transition-all">🔍 اسکنر IP</a>
                <a href="/${sysConfig.apiRoute}/subscribe" class="px-6 py-3 rounded-xl font-bold bg-slate-800 hover:bg-slate-700 text-white transition-all">🔗 اشتراک</a>
                <a href="/${sysConfig.apiRoute}/protocols" class="px-6 py-3 rounded-xl font-bold bg-slate-800 hover:bg-slate-700 text-white transition-all">📡 پروتکل‌ها</a>
            </div>
        </div>
    </div>
</body>
...
function getScannerPage() {
    return `<!DOCTYPE html>
<html lang="fa">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>اسکنر IP - TaaKaa-Xi</title>
<link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;700;900&display=swap" rel="stylesheet">
<style>body{font-family:'Vazirmatn',sans-serif;background:#0a0a0f;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;padding:20px}.container{max-width:700px;width:100%;background:rgba(26,26,46,0.85);border-radius:30px;padding:40px;border:1px solid rgba(255,107,0,0.3)}h1{color:#ff6b00;text-align:center}.scan-btn{background:#ff6b00;color:#fff;border:none;padding:14px 30px;border-radius:50px;font-size:18px;font-weight:700;cursor:pointer;width:100%;transition:all .3s}.scan-btn:hover{background:#ff8c42}.results{margin-top:30px;background:rgba(0,0,0,0.3);border-radius:15px;padding:20px;max-height:400px;overflow-y:auto}.ip-item{display:flex;justify-content:space-between;padding:10px 15px;border-bottom:1px solid rgba(255,255,255,0.05)}.ip-item:last-child{border-bottom:none}.status{color:#4ade80}</style>
</head>
<body>
<div class="container">
<h1>🔍 اسکنر IP</h1>
<p style="text-align:center;color:#aaa;margin-bottom:30px">اسکن آی‌پی‌های سالم برای اتصال</p>
<button class="scan-btn" onclick="scanIPs()">شروع اسکن</button>
<div id="results" class="results"><p style="text-align:center;color:#666">برای شروع کلیک کنید</p></div>
</div>
<script>
async function scanIPs() {
    const res = document.getElementById('results');
    res.innerHTML = '<p style="text-align:center;color:#ff6b00">در حال اسکن...</p>';
    const domains = ['www.speedtest.net','grok.com','chatgpt.com','snapp.ir','digikala.com','aparat.com'];
    let ips = [];
    for (const d of domains) {
        try {
            const r = await fetch('https://cloudflare-dns.com/dns-query?name='+d+'&type=A', {headers:{'accept':'application/dns-json'}});
            const data = await r.json();
            if(data.Answer) data.Answer.forEach(a => { if(a.type===1 && a.data) ips.push(a.data); });
        } catch(e) {}
    }
    ips = [...new Set(ips)];
    if(ips.length===0) { res.innerHTML='<p style="text-align:center;color:#f87171">هیچ آی‌پی پیدا نشد</p>'; return; }
    res.innerHTML = ips.map(ip => '<div class="ip-item"><span>'+ip+'</span><span class="status">✅ سالم</span></div>').join('');
}
</script>
</body></html>`;
}
...
<html lang="fa">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>اشتراک - TaaKaa-Xi</title>
<link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;700;900&display=swap" rel="stylesheet">
<style>body{font-family:'Vazirmatn',sans-serif;background:#0a0a0f;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;padding:20px}.container{max-width:600px;width:100%;background:rgba(26,26,46,0.85);border-radius:30px;padding:40px;border:1px solid rgba(255,107,0,0.3)}h1{color:#ff6b00;text-align:center}.link-box{background:rgba(0,0,0,0.3);border-radius:15px;padding:15px;word-break:break-all;font-family:monospace;font-size:12px;color:#4ade80}.copy-btn{background:#ff6b00;color:#fff;border:none;padding:10px 20px;border-radius:10px;cursor:pointer;margin-top:10px;font-weight:700}.copy-btn:hover{background:#ff8c42}</style>
</head>
<body>
<div class="container">
<h1>🔗 لینک اشتراک</h1>
<p style="text-align:center;color:#aaa">لینک اشتراک خود را کپی کنید</p>
<div class="link-box" id="subLink">https://${sysConfig.apiRoute}/?sub=Default</div>
<button class="copy-btn" onclick="copyLink()">📋 کپی</button>
</div>
<script>
const link = document.getElementById('subLink');
function copyLink() { navigator.clipboard.writeText(link.innerText); alert('کپی شد!'); }
</script>
</body></html>`;
}

function getProtocolsPage() {
    return `<!DOCTYPE html>
<html lang="fa">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>پروتکل‌ها - TaaKaa-Xi</title>
<link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;700;900&display=swap" rel="stylesheet">
<style>body{font-family:'Vazirmatn',sans-serif;background:#0a0a0f;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;padding:20px}.container{max-width:700px;width:100%;background:rgba(26,26,46,0.85);border-radius:30px;padding:40px;border:1px solid rgba(255,107,0,0.3)}h1{color:#ff6b00;text-align:center}.protocol{background:rgba(0,0,0,0.3);border-radius:15px;padding:15px;margin:10px 0;border-right:4px solid #ff6b00}</style>
</head>
<body>
<div class="container">
<h1>📡 پروتکل‌های پشتیبانی شده</h1>
<div class="protocol"><strong>VLESS</strong> - پروتکل سبک و سریع</div>
<div class="protocol"><strong>Trojan</strong> - پروتکل امن با استتار</div>
<div class="protocol"><strong>WebSocket</strong> - انتقال داده از طریق WebSocket</div>
<div class="protocol"><strong>gRPC</strong> - پشتیبانی از gRPC</div>
<p style="text-align:center;color:#666;margin-top:30px">کانال تلگرام: ${TELEGRAM_CHANNEL}</p>
</div>
</body></html>`;
}
...
<html lang="fa">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>تیم - TaaKaa-Xi</title>
<link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@400;700;900&display=swap" rel="stylesheet">
<style>body{font-family:'Vazirmatn',sans-serif;background:#0a0a0f;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;padding:20px}.container{max-width:700px;width:100%;background:rgba(26,26,46,0.85);border-radius:30px;padding:40px;border:1px solid rgba(255,107,0,0.3)}h1{color:#ff6b00;text-align:center}.member{display:flex;align-items:center;gap:15px;background:rgba(0,0,0,0.3);border-radius:15px;padding:15px;margin:10px 0}.avatar{width:50px;height:50px;border-radius:50%;background:#ff6b00;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;color:#fff}</style>
</head>
<body>
<div class="container">
<h1>👥 تیم توسعه TaaKaa-Xi</h1>
<div class="member"><div class="avatar">T</div><div><strong>تیم TaaKaa</strong><br><span style="color:#aaa">توسعه‌دهنده اصلی</span></div></div>
<div class="member"><div class="avatar">X</div><div><strong>تیم Xi</strong><br><span style="color:#aaa">توسعه‌دهنده بک‌اند</span></div></div>
<p style="text-align:center;color:#666;margin-top:30px">کانال تلگرام: ${TELEGRAM_CHANNEL}</p>
</div>
</body></html>`;
}
...
// ============================================================
// Build Functions - ساخت کانفیگ‌ها (URI, YAML, Clash, Sing-Box)
// ============================================================
...
function getConfigNameWithFlag(type, profileName, port, hostName, ip, proxyIp = null) {
    let prefix = sysConfig.namePrefix || "TaaKaa";
    let strategy = sysConfig.nameStrategy || "default";
    let cleanName = profileName === "Default" ? "" : `-${profileName}`;
    let typeLab = type === "alpha" ? "V" : "T";
...
function getConfigName(type, profileName, port, hostName, ip, proxyIp = null) {
    return getConfigNameWithFlag(type, profileName, port, hostName, ip, proxyIp);
}
            ...
            effectivePorts.forEach(port => {
                ...
                ips.forEach(ip => {
                    ...
                    if (effectiveMode === "alpha" || effectiveMode === "both") {
                        lines.push(`${getAlpha()}://${p.id}@${ip}:${port}?${extBase}#${encodeURIComponent(vName)}`);
                    }
                    if (effectiveMode === "beta" || effectiveMode === "both") {
                        lines.push(`${getBeta()}://${p.id}@${ip}:${port}?${extBase}#${encodeURIComponent(tName)}`);
                    }
                });
            });
        });
    ...
    async fetch(request, env, ctx) {
        try {
            ...
            const routes = {
                ...
                dash: `/${encodeURI(sysConfig.apiRoute)}/taakaa`,
                ...
                scanner: `/${encodeURI(sysConfig.apiRoute)}/scanner`,
                subscribe: `/${encodeURI(sysConfig.apiRoute)}/subscribe`,
                protocols: `/${encodeURI(sysConfig.apiRoute)}/protocols`,
                owners: `/${encodeURI(sysConfig.apiRoute)}/owners`,
            };
            ...
            // مسیرهای جدید
            if (reqPath === routes.scanner) {
                return new Response(getScannerPage(), { headers: { "Content-Type": "text/html;charset=utf-8" } });
            }
            if (reqPath === routes.subscribe) {
                return new Response(getSubscribePage(), { headers: { "Content-Type": "text/html;charset=utf-8" } });
            }
            if (reqPath === routes.protocols) {
                return new Response(getProtocolsPage(), { headers: { "Content-Type": "text/html;charset=utf-8" } });
            }
            if (reqPath === routes.owners) {
                return new Response(getOwnersPage(), { headers: { "Content-Type": "text/html;charset=utf-8" } });
            }
            ...
            if (!isTelemetryStream) {
                if (reqPath === routes.dash) {
                    return new Response(getDashboardUI(env[DB_NAME] !== undefined), { headers: { "Content-Type": "text/html;charset=utf-8" } });
                }
