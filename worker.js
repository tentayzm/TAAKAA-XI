// ============================================================
// Project TaaKaa-Xi - IoT Device Telemetry Gateway
...
async function d1Init(env) {
    if(env.TAAKAA_DB && !env.TAAKAA_DB_INITIALIZED) {
        try { await env.TAAKAA_DB.prepare("CREATE TABLE IF NOT EXISTS kv_store (key TEXT PRIMARY KEY, value TEXT)").run(); env.TAAKAA_DB_INITIALIZED = true; } catch(e) { env.TAAKAA_DB_INITIALIZED = true; }
    }
}
async function d1Get(env, key) {
    if(!env.TAAKAA_DB) return null;
    await d1Init(env);
    try { const { results } = await env.TAAKAA_DB.prepare("SELECT value FROM kv_store WHERE key = ?").bind(key).all(); if(results && results.length > 0) return results[0].value; } catch(e) {}
    return null;
}
async function d1Put(env, key, value) {
    if(!env.TAAKAA_DB) return;
    await d1Init(env);
    try { await env.TAAKAA_DB.prepare("INSERT INTO kv_store (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value").bind(key, value).run(); } catch(e) {}
}
        ...
        try {
            ...
            if (!isTelemetryStream) {
                if (reqPath === routes.dash) {
                    return new Response(getDashboardUI(env.TAAKAA_DB !== undefined), { headers: { "Content-Type": "text/html;charset=utf-8" } });
                }
...
<head>
    ...
    <style>
        body {
            ...
            background: linear-gradient(135deg, #0d1117 0%, #1a120e 50%, #0d1117 100%) !important;
            color: #f1f5f9;
        }
        .premium-card {
            background: linear-gradient(145deg, rgba(30, 20, 10, 0.85), rgba(13, 17, 23, 0.85)) !important;
...
function getDashboardUI(hasDB) {
  ...
  <head>
      ...
      <style>
          ...
          html.dark .bg-white, html.dark .bg-slate-50, html.dark .bg-indigo-50, html.dark .bg-darkcard {
              background: linear-gradient(145deg, rgba(20, 12, 8, 0.85), rgba(13, 17, 23, 0.85)) !important;
              border: 1px solid rgba(251, 146, 60, 0.35) !important;
              box-shadow: 0 10px 40px rgba(0, 0, 0, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.05) !important;
          }
          html.dark aside {
              background: rgba(13, 17, 23, 0.6) !important;
              border-inline-end: 1px solid rgba(251, 146, 60, 0.25) !important;
          ...
          html:not(.dark) #github-link-btn, html:not(.dark) #lang-toggle, html:not(.dark) #telegram-link-btn {
              background-color: #ffffff !important;
              border-color: #cbd5e1 !important;
              color: #475569 !important;
          }
          html:not(.dark) #github-link-btn:hover, html:not(.dark) #lang-toggle:hover, html:not(.dark) #telegram-link-btn:hover {
              border-color: #cbd5e1 !important;
              color: #1e293b !important;
          }
          html:not(.dark) .nav-item.active { 
               background: linear-gradient(90deg, rgba(249, 115, 22, 0.1), transparent) !important; 
               color: #ea580c !important; 
          ...
          html:not(.dark) .text-indigo-400 { color: #ea580c !important; }
          html:not(.dark) .text-violet-400 { color: #7c3aed !important; }
          
          .nav-item.active { 
              background: linear-gradient(90deg, rgba(249, 115, 22, 0.2), transparent) !important; 
          ...
          .dark .nav-item.active { 
              background: linear-gradient(90deg, rgba(249, 115, 22, 0.2), transparent) !important; 
              color: #fdba74 !important; 
              border-inline-start: 4px solid #fb923c !important; 
          }
          ...
          .mobile-nav-item.active { color: #fb923c; }
          .dark .mobile-nav-item.active { color: #fb923c; }
      </style>
  </head>
  <body class="text-slate-800 dark:text-slate-200 h-[100dvh] flex flex-col md:flex-row overflow-hidden selection:bg-primary selection:text-white transition-colors duration-300 bg-slate-50 dark:bg-darkbg">
      ...
      <div class="fixed top-4 end-4 md:top-5 md:end-5 flex items-center gap-2 z-50">
          <span id="top-version-badge" class="px-3 py-1.5 rounded-xl text-[11px] font-mono font-bold" style="background:rgba(249,115,22,0.12);border:1px solid rgba(249,115,22,0.25);color:#fb923c;">v${CURRENT_VERSION}</span>
          <a href="https://github.com/tentayzm/TaaKaa-Xi" id="github-link-btn" target="_blank" class="p-2 rounded-xl transition-all" style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);color:#94a3b8;" onmouseover="this.style.color='#fb923c';this.style.borderColor='rgba(249,115,22,0.4)'" onmouseout="this.style.color='#94a3b8';this.style.borderColor='rgba(255,255,255,0.1)'">
          ...
          <a href="https://t.me/TaaKaaOrg" id="telegram-link-btn" target="_blank" class="p-2 rounded-xl transition-all" style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);color:#94a3b8;" onmouseover="this.style.color='#fb923c';this.style.borderColor='rgba(249,115,22,0.4)'" onmouseout="this.style.color='#94a3b8';this.style.borderColor='rgba(255,255,255,0.1)'">
          ...
          <button onclick="toggleLang()" id="lang-toggle" class="px-3 py-1.5 rounded-xl text-sm font-bold transition-all" style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);color:#e2e8f0;" onmouseover="this.style.borderColor='rgba(249,115,22,0.4)';this.style.color='#fb923c'" onmouseout="this.style.borderColor='rgba(255,255,255,0.1)';this.style.color='#e2e8f0'">EN</button>
      ...
      <!-- LOGIN SCREEN -->
      <div id="login-box" class="absolute inset-0 flex items-center justify-center p-4 z-40 overflow-hidden" style="background:linear-gradient(135deg,#0d1117 0%,#1a120e 50%,#0d1117 100%);">
          <div class="absolute pointer-events-none" style="width:500px;height:500px;top:-100px;left:-150px;background:radial-gradient(circle,rgba(249,115,22,0.12) 0%,transparent 65%);"></div>
          <div class="absolute pointer-events-none" style="width:400px;height:400px;bottom:-80px;right:-100px;background:radial-gradient(circle,rgba(249,115,22,0.1) 0%,transparent 65%);"></div>
          <div class="relative w-full max-w-sm">
              <style>
                  @keyframes pulse-ring{0%{transform:scale(1);opacity:0.5}100%{transform:scale(1.7);opacity:0}}
                  @keyframes shimmer{0%{left:-100%}100%{left:100%}}
                  .lock-pulse::before,.lock-pulse::after{content:'';position:absolute;inset:-8px;border-radius:50%;border:1px solid rgba(249,115,22,0.35);animation:pulse-ring 2.5s ease-out infinite;}
              ...
              <div class="text-center mb-8">
                  <div class="relative inline-flex items-center justify-center mb-5">
                      <div class="lock-pulse relative w-20 h-20 rounded-3xl flex items-center justify-center" style="background:linear-gradient(145deg,rgba(249,115,22,0.25),rgba(249,115,22,0.08));border:1px solid rgba(249,115,22,0.45);box-shadow:0 0 40px rgba(249,115,22,0.25),inset 0 1px 0 rgba(255,255,255,0.08);">
                          <svg class="w-9 h-9" style="color:#fb923c" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path></svg>
                      </div>
                  </div>
                  <h2 class="text-3xl font-black" style="color:#f1f5f9;" data-i18n="title">TaaKaa-Xi Gateway</h2>
                  <p class="text-sm mt-2" style="color:#64748b;">Sign in to manage your gateway</p>
              </div>
              <div class="rounded-3xl p-px" style="background:linear-gradient(145deg,rgba(249,115,22,0.45),rgba(249,115,22,0.08) 50%,rgba(249,115,22,0.3));box-shadow:0 25px 60px rgba(0,0,0,0.5);">
                  <div class="rounded-3xl p-8" style="background:linear-gradient(145deg,rgba(20,12,8,0.98),rgba(13,17,23,0.98));">
                      ...
                      <div class="mb-5">
                          ...
                          <div class="relative">
                              <div class="absolute inset-y-0 start-0 flex items-center ps-4" style="color:rgba(249,115,22,0.7);">
                                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"></path></svg>
                              </div>
                              <input type="password" id="pwd" data-i18n="pass_ph" placeholder="Enter your password" class="w-full ps-11 pe-12 py-3.5 text-sm rounded-2xl outline-none transition-all" style="background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);color:#e2e8f0;" onfocus="this.style.borderColor='rgba(249,115,22,0.6)';this.style.background='rgba(249,115,22,0.06)';this.style.boxShadow='0 0 0 3px rgba(249,115,22,0.1)'" onblur="this.style.borderColor='rgba(255,255,255,0.1)';this.style.background='rgba(255,255,255,0.04)';this.style.boxShadow='none'">
                              <button type="button" onclick="const n=document.getElementById('pwd');n.type=n.type==='password'?'text':'password'" class="absolute inset-y-0 end-0 flex items-center px-4 transition-colors" style="color:rgba(249,115,22,0.5);" onmouseover="this.style.color='#fb923c'" onmouseout="this.style.color='rgba(249,115,22,0.5)'">
                      ...
                      <button onclick="doLogin()" class="btn-shimmer w-full py-3.5 rounded-2xl font-bold text-sm relative overflow-hidden transition-all" style="background:linear-gradient(135deg,#f97316,#ea580c);color:white;box-shadow:0 4px 24px rgba(249,115,22,0.4),inset 0 1px 0 rgba(255,255,255,0.1);" onmouseover="this.style.boxShadow='0 6px 32px rgba(249,115,22,0.6),inset 0 1px 0 rgba(255,255,255,0.1)';this.style.transform='translateY(-1px)'" onmouseout="this.style.boxShadow='0 4px 24px rgba(249,115,22,0.4),inset 0 1px 0 rgba(255,255,255,0.1)';this.style.transform='translateY(0)'" data-i18n="login_btn">
                          Sign In
                      </button>
                  </div>
              </div>
      ...
      <div id="dash-box" class="hidden w-full h-full flex-col md:flex-row relative">
          ...
          <aside class="hidden md:flex w-64 bg-white dark:bg-darkcard border-e border-slate-200 dark:border-darkborder flex-col z-20 shrink-0">
              <div class="flex items-center p-6 border-b border-slate-100 dark:border-darkborder/50">
                  <div class="w-10 h-10 rounded-xl bg-orange-50 dark:bg-orange-900/40 text-primary flex items-center justify-center me-3 shrink-0"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg></div>
                  <div class="flex flex-col">
                      <h1 class="font-black text-xl leading-none" data-i18n="title">TaaKaa-Xi</h1>
                  ...
                  <div class="max-w-4xl mx-auto space-y-6 fade-in">
                      ...
                      <div id="update-alert-banner" class="hidden bg-gradient-to-r from-amber-500/10 to-primary/10 border-2 border-amber-300 dark:border-amber-950/20 rounded-3xl p-6 shadow-md flex-col items-center justify-between gap-4 fade-in">
                          ...
                          <div class="w-full mt-2 text-center">
                              <a id="update-github-link" href="https://github.com/tentayzm/TaaKaa-Xi" target="_blank" class="text-xs text-slate-400 hover:text-amber-500 transition-colors underline" data-i18n="view_github">View on GitHub</a>
                          </div>
                      </div>
                      ...
                      <div id="view-overview" class="space-y-3 md:space-y-6 block">
                          ...
                          <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-4">
                              ...
                              <div class="bg-white dark:bg-darkcard rounded-xl md:rounded-2xl p-3 md:p-5 shadow-sm border border-slate-200 dark:border-darkborder">
                                  <div class="flex items-center gap-2 md:gap-3 mb-2 md:mb-3">
                                      <div class="p-1.5 md:p-2.5 bg-orange-500/10 text-orange-500 rounded-lg md:rounded-xl"><svg class="w-4 h-4 md:w-5 md:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path></svg></div>
                      ...
                      <div id="view-settings" class="hidden">
                          <div class="bg-white dark:bg-darkcard rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-darkborder grid grid-cols-1 md:grid-cols-2 gap-5">
                              ...
                              <div class="space-y-1 md:col-span-2">
                                  ...
                                  <input type="text" id="cfg-github-repo" placeholder="tentayzm/TaaKaa-Xi" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-darkborder bg-slate-50 dark:bg-slate-800 focus:border-primary outline-none text-sm">
                      ...
                      <div id="view-advanced" class="hidden space-y-6">
                          ...
                          <div class="bg-white dark:bg-darkcard rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-darkborder mb-4">
                              <div class="flex items-center justify-between mb-4">
                                  ...
                                  <span class="text-xs bg-orange-100 dark:bg-orange-900/50 text-orange-700 dark:text-orange-300 px-2 py-1 rounded-md font-bold" id="ip-count-badge">1 Config Set</span>
                              </div>
                          ...
                          <!-- Slave Nodes Section -->
                          <div class="bg-orange-50 dark:bg-orange-900/20 rounded-3xl p-6 shadow-sm border border-orange-100 dark:border-orange-900/50 relative overflow-hidden">
                              <div class="absolute top-0 end-0 bg-orange-100 dark:bg-orange-900/40 px-3 py-1 text-[10px] font-bold text-orange-500 dark:text-orange-400 rounded-bl-xl">CLUSTER</div>
                              <div class="flex items-center justify-between mb-2">
                                  <h3 class="text-sm uppercase font-black text-orange-800 dark:text-orange-300 tracking-wider flex items-center">
                              ...
                              <p class="text-xs text-orange-600/80 dark:text-orange-300/70 mb-4 leading-relaxed" data-i18n="slave_desc">Enter your other worker Domains (one per line). Master will push settings and users to them automatically, and include them in load-balanced subscriptions!</p>
                              <div class="relative">
                                  <textarea id="cfg-nodes" rows="3" placeholder="node1.worker.dev&#10;node2.domain.com" class="w-full px-4 py-3 pb-12 rounded-xl border border-orange-200 dark:border-orange-800/50 bg-white dark:bg-slate-900 focus:border-orange-500 focus:ring-1 outline-none font-mono text-sm resize-none scrollbar-hide text-slate-700 dark:text-slate-300 placeholder:text-orange-200 dark:placeholder:text-orange-800/50"></textarea>
                                  <div class="absolute bottom-3 end-3">
                                      <button onclick="forceSyncNodes()" type="button" class="px-3 py-1.5 bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold rounded-lg transition-colors flex items-center shadow-sm">
                          ...
                          <div class="bg-white dark:bg-darkcard rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-darkborder grid grid-cols-1 md:grid-cols-2 gap-5 mt-6">
                              ...
                              <div class="space-y-1 text-start md:col-span-2">
                                  ...
                                  <input type="text" id="cfg-cf-worker" placeholder="e.g. taakaa" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-darkborder bg-slate-50 dark:bg-slate-800 focus:border-primary outline-none text-sm font-mono">
                                  ...
                                  <div id="cf-helper-guide" class="hidden mt-3 p-4 bg-slate-50/50 dark:bg-slate-900/30 border border-slate-200 dark:border-darkborder rounded-2xl text-[11px] space-y-4 text-start leading-relaxed">
                                      ...
                                      <div class="space-y-1 pb-3 border-b border-dashed border-slate-200 dark:border-darkborder">
                                          ...
                                          <ol class="list-decimal list-inside space-y-1 text-slate-500 dark:text-slate-400">
                                              ...
                                              <li><strong>Worker Script Name:</strong> Go to <strong class="text-slate-800 dark:text-slate-300">Compute &gt; Workers & Pages</strong> in Cloudflare. Copy your worker's name (e.g. <code>taakaa</code> or <code>taakaa-gateway</code>).</li>
                                          </ol>
                                      </div>
                                      <!-- Persian Section -->
                                      <div id="cf-helper-fa" class="space-y-1" style="direction: rtl;">
                                          ...
                                          <ol class="list-decimal list-inside space-y-1 text-slate-500 dark:text-slate-400">
                                              ...
                                              <li><strong>نام کارگر (Worker Name):</strong> از منوی کناری کلودفلر به بخش <strong class="text-slate-800 dark:text-slate-300">Compute &gt; Workers & Pages</strong> رفته و نام ورکر خود را عینا بنویسید (مثلاً <code>taakaa</code>).</li>
                                          </ol>
                                      </div>
                                  </div>
                              </div>
    ...
    <div id="edit-user-proxy-ips-wrap" class="flex flex-wrap gap-2 mt-1 text-slate-500"></div>
      ...
      <div id="modal-version-update" class="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[101] hidden items-center justify-center p-4">
          <div class="bg-white dark:bg-darkcard rounded-3xl p-8 max-w-lg w-full shadow-2xl border border-slate-200 dark:border-darkborder relative overflow-hidden transform transition-all duration-300">
              <div class="absolute top-0 right-0 left-0 h-2 bg-gradient-to-r from-orange-500 via-primary to-emerald-500"></div>
              <div class="flex items-center justify-between mb-6">
                  <div class="flex items-center gap-2.5">
                      ...
                      <div>
                          ...
                          <span id="modal-version-badge" class="text-[10px] font-bold px-2 py-0.5 bg-orange-500 text-white rounded-full tracking-wide"></span>
                      </div>
                  </div>
      ...
      <script>
          ...
          function obfuscateCode(srcText) {
              ...
              cleanCode = cleanCode.replace(/export\\s+default\\s+/g, 'const _0xTaaKaaModule = ');
              cleanCode += '\\nreturn _0xTaaKaaModule;';
              ...
              const finalLoaderCode = rawImportsStr + '\\n\\n' +
                  '// TaaKaa-Xi Gateway - Obfuscated Loader Context (v1.0 Optimized)\\n' +
                  'const _0xTaaKaaPayload = "' + hexOutput + '";\\n' +
                  'const _0xTaaKaaKey = ' + randKey + ';\\n\\n' +
                  'const _0xTaaKaaBytes = new Uint8Array((_0xTaaKaaPayload.match(/.{1,2}/g) || []).map(x => parseInt(x, 16) ^ _0xTaaKaaKey));\\n' +
                  'const _0xTaaKaaCode = new TextDecoder().decode(_0xTaaKaaBytes);\\n' +
                  'const _0xTaaKaaRuntime = new Function(' + bindingNames.map(name => '"' + name + '"').join(', ') + ', _0xTaaKaaCode)(' + bindingNames.join(', ') + ');\\n\\n' +
                  'export default _0xTaaKaaRuntime;';
          ...
          const i18n = {
              en: {
                  title: "TaaKaa-Xi Gateway", pass_ph: "Master Key", login_btn: "Authenticate", err_pass: "Access Denied", missing_db: "⚠️ TAAKAA_DB namespace missing! Settings won't save.",
              ...
              fa: {
                  title: "دروازه تاکا-ایکس", pass_ph: "کلید اصلی", login_btn: "ورود به سیستم", err_pass: "دسترسی مسدود شد", missing_db: "⚠️ فضای پایگاه داده یافت نشد! تنظیمات ذخیره نمی‌شوند.",
          ...
          const CHANGELOG_DATA = {
              "1.0": {
                  headline: { en: "Initial Release of TaaKaa-Xi Gateway", fa: "نسخه اولیه دروازه تاکا-ایکس" },
                  added: [
                      { en: "Initial public release with full core functionality", fa: "انتشار اولیه با تمام قابلیت‌های اصلی" },
                      { en: "Support for VLESS and Trojan protocols", fa: "پشتیبانی از پروتکل‌های VLESS و Trojan" },
                      { en: "Multi-user management with traffic/expiry controls", fa: "مدیریت چند کاربره با کنترل ترافیک و انقضا" },
                      { en: "Clash, Sing-box, and URI subscription formats", fa: "فرمت‌های اشتراک Clash، Sing-box و URI" },
                      { en: "Telegram bot integration for management", fa: "یکپارچه‌سازی ربات تلگرام برای مدیریت" },
                      { en: "Clean IP and proxy IP rotation", fa: "چرخش آی‌پی تمیز و آی‌پی پروکسی" },
                  ],
                  fixed: [],
                  improved: [],
                  notes: []
          ...
          window.addEventListener('DOMContentLoaded', () => {
              let savedSession = localStorage.getItem('taakaa_session');
              if (savedSession) {
                  try {
                      ...
                      if (parsed && parsed.expiry && Date.now() < parsed.expiry) {
                          ...
                          localStorage.removeItem('taakaa_session');
                      }
                  } catch(e){}
              }
              checkVersionPopup();
          ...
          function checkVersionPopup() {
              const popupKey = \`taakaa_shown_v\${CURRENT_VERSION}\`;
        ...
        async function loadDashboard() {
            try {
                ...
                if (logsData.success && logsData.logs && logsData.logs.length > 0) {
                    ...
                    logsData.logs.slice(0, 8).forEach(log => {
                        const dateStr = new Date(log.ts).toLocaleString('en-US', {hour12: false});
                        const typeColors = { 'Auth Success': 'bg-emerald-500', 'Auth Failed': 'bg-red-500', 'User Created': 'bg-blue-500', 'User Deleted': 'bg-red-500', 'User Toggled': 'bg-amber-500', 'User Updated': 'bg-orange-500', 'User Auto-Disabled': 'bg-red-500', 'Traffic Reset': 'bg-cyan-500', 'Config Changed': 'bg-violet-500' };
          ...
          function exportConfig() {
              ...
              dlAnchor.setAttribute("download", "taakaa-gateway-config.json");
          ...
          async function doLogin(silent = false) {
              ...
              try {
                  ...
                  if (data.success) {
                      ...
                      localStorage.setItem('taakaa_session', JSON.stringify({ key: pass, expiry: Date.now() + 30 * 60 * 1000 }));
                      ...
                      document.getElementById('cfg-github-repo').value = conf.githubRepo || 'tentayzm/TaaKaa-Xi';
                      ...
                      window.taakaaConfig = JSON.parse(JSON.stringify(conf));
                      window.taakaaUsage = data.sysUsage || {};
                      window.taakaaProfiles = data.profiles || [];
                      renderUsersTable();
    ...
    <div class="transition-all duration-300" style="max-height:0;overflow:hidden;" data-accordion-content>
        <div class="space-y-3 px-5 md:px-6 pb-5 md:pb-6">
            ...
            <div class="mt-2">
                <button data-id="\${p.id}" data-name="\${p.name}" onclick="handleQR(this)" class="w-full flex items-center justify-center p-2.5 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-200 dark:border-darkborder rounded-xl transition-all gap-1.5 text-[11px] font-bold text-slate-600 dark:text-slate-400">
                    ...
                    <span>Show QR Code</span>
                      ...
                      else { localStorage.removeItem('taakaa_session'); }
                  }
          ...
          async function doSave() {
              ...
              const payload = {
                  ...
                  config: {
                     ...
                     payload.config.users = (window.taakaaConfig.users || []).map(u => {
          ...
          function renderUsersTable() {
              ...
              if(!tbl) return;
              let users = window.taakaaConfig?.users || [];
              let usage = window.taakaaUsage || {};
              ...
              users.forEach((u, i) => {
                  ...
                  let linkHtml = \`<button onclick="copyData('sync-\${u.id}')" class="text-primary hover:text-orange-700 bg-orange-50 hover:bg-orange-100 dark:bg-orange-900/30 dark:hover:bg-orange-800/50 p-2 rounded-lg" title="\${linkTitle}">🔗</button>\`;
                  ...
                  let editBtnHtml = \`<button onclick="editUser('\${u.id}')" class="text-orange-500 hover:text-orange-700 bg-orange-50 hover:bg-orange-100 dark:bg-orange-900/30 dark:hover:bg-orange-800/50 p-2 rounded-lg" title="\${editTitle}">✏️</button>\`;
                  ...
                  let rawSync = window.taakaaProfiles?.find(p => p.id === u.id)?.sync || '';
                  if (rawSync) {
                  ...
                  tr.innerHTML = \`
                      <td class="px-4 py-4 font-bold text-slate-700 dark:text-slate-300">\${u.name} \${u.isPaused ? (isAutoDisabled ? '🚫' : '⏸️') : (isExp ? '🔴' : '🟢')}
                          <div class="flex flex-wrap gap-1 mt-1">
                              ...
                              \${u.userMode ? \`<span class="text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-orange-100 dark:bg-orange-900/40 text-orange-600 dark:text-orange-300">\${u.userMode === 'alpha' ? 'VLESS' : u.userMode === 'beta' ? 'Trojan' : 'VLESS+Trojan'}</span>\` : ''}
                      ...
                      <td class="px-4 py-4 text-slate-600 dark:text-slate-400 font-mono">
                          <div class="flex flex-col gap-1.5">
                              ...
                              <span class="text-[11px] opacity-70 flex items-center gap-1"><span class="w-1.5 h-1.5 rounded-full bg-orange-500"></span>\${dailyLabel} \${userDReqs} \${rLabel} (\${(userDReqs/6050).toFixed(2)} \${i18n[lang]?.ov_gb_unit || 'GB'}) / \${u.limitDailyReq ? (u.limitDailyReq + ' ' + rLabel + ' (' + (u.limitDailyReq/6050).toFixed(2) + ' ' + (i18n[lang]?.ov_gb_unit || 'GB') + ')') : \`\${unlimitedTxt}\`} (\${perD})</span>
                              
                              \${u.limitDailyReq ? \`
                              <div class="w-full bg-slate-100 dark:bg-slate-800/80 h-1.5 rounded-full overflow-hidden mt-0.5">
                                  <div class="bg-orange-500 h-full rounded-full transition-all duration-500" style="width: \${perD}"></div>
                              </div>
                              \` : ''}
                          </div>
                      </td>
          ...
          function deleteUser(uuid) {
              ...
              if(window.taakaaConfig && window.taakaaConfig.users) {
                  window.taakaaConfig.users = window.taakaaConfig.users.filter(u => u.id !== uuid);
              }
              // Automatically sync
              renderUsersTable();
              doSaveDirectly();
          ...
          function togglePauseUser(uuid) {
              if(window.taakaaConfig && window.taakaaConfig.users) {
                  let usr = window.taakaaConfig.users.find(u => u.id === uuid);
                  if (usr) {
          ...
          function getGlobalPorts() {
              return (window.taakaaConfig && window.taakaaConfig.socketPorts)
                  ? window.taakaaConfig.socketPorts.split(',').map(s=>s.trim()).filter(Boolean)
                  : ['443'];
          }

          function getGlobalMode() {
              return (window.taakaaConfig && window.taakaaConfig.mode) ? window.taakaaConfig.mode : 'alpha';
          }
          ...
          function openAddUserModal() {
              ...
              buildIPCheckboxes("add-user-clean-ips-wrap", "", (window.taakaaConfig?.cleanIps||"").split(/[\\s,;]+/).map(s=>s.trim()).filter(Boolean));
              buildIPCheckboxes("add-user-proxy-ips-wrap", "", (window.taakaaConfig?.backupRelay||"").split(/[\\s,;]+/).map(s=>s.trim()).filter(Boolean));
          }
...
function buildPortCheckboxes(wrapId, selectedPorts) {
          ...
          function commitAddUser() {
              ...
              if(!window.taakaaConfig) window.taakaaConfig = {};
              if(!window.taakaaConfig.users) window.taakaaConfig.users = [];

              if(window.taakaaConfig.users.some(u => u.name.trim().toLowerCase() === name.toLowerCase())) {
              ...
              window.taakaaConfig.users.push(u);
          ...
          function editUser(uuid) {
              if(!window.taakaaConfig || !window.taakaaConfig.users) return;
              let u = window.taakaaConfig.users.find(usr => usr.id === uuid);
              if(!u) return;
              ...
              document.getElementById('edit-user-daily-reqs').value = u.limitDailyReq? (u.limitDailyReq / 6000).toFixed(2): '';
                            const globalCleanIps = (window.taakaaConfig?.cleanIps||"").split(/[\\r\\n,;]+/).map(s=>s.trim()).filter(Boolean);
              ...
              const globalProxyIps = (window.taakaaConfig?.backupRelay||"").split(/[\\r\\n,;]+/).map(s=>s.trim()).filter(Boolean);
          ...
          function commitEditUser() {
              ...
              if(window.taakaaConfig.users.some(u => u.id !== uuid && u.name.trim().toLowerCase() === name.toLowerCase())) {
                  alert(lang === 'fa' ? 'این نام قبلاً استفاده شده است' : 'This name is already taken');
                  return;
              }
          ...
          async function doSaveDirectly() {
              ...
              try {
                  const res = await fetch(baseRoute + '/api/sync', {
                      method: 'POST',
                      headers: {'Content-Type': 'application/json'},
                      body: JSON.stringify({ key: sessionKey, config: window.taakaaConfig })
                  });
                  if(res.ok) {
          ...
          async function checkUpdate() {
              try {
                  ...
                  if (data.success && data.updateAvailable) {
                      ...
                      showUpdateBanner((document.getElementById('cfg-github-repo')?.value || window.taakaaConfig?.githubRepo || 'tentayzm/TaaKaa-Xi').replace('https://github.com/', '').replace('http://github.com/', '').trim(), data.latest);
                  }
          ...
          async function doUpdate() {
              ...
              let latestCode = null;
              try {
                  const repo = (document.getElementById('cfg-github-repo')?.value || window.taakaaConfig?.githubRepo || 'tentayzm/TaaKaa-Xi').replace('https://github.com/', '').replace('http://github.com/', '').trim();
    ...
    document.getElementById("net-datetime").innerText = custom;
          ...
          document.addEventListener('DOMContentLoaded', () => {
              const cached = localStorage.getItem('taakaa_session');
              if(cached) {
                  try {
                  ...
                  } catch(e) { localStorage.removeItem('taakaa_session'); }
              }
          });
      </script>
  </body>
